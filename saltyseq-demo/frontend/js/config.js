...
MS.STATE_LABELS = {
  Salt_High:'Mặn cao', Salt_Mod:'Mặn vừa', Salt_Low:'Mặn thấp',
...
MS.stressVar = function(rate) {
  if (rate >= 0.15) return 'var(--danger-5)';
  if (rate >= 0.05) return 'var(--warn-5)';
  return 'var(--safe-5)';
};
...
MS.chipStyle = function(state) {
  const map = {
    ...
    Salt_Mod:       { bg:'var(--warn-lt)',    bd:'var(--warn-bd)',   tx:'var(--warn)'     },
